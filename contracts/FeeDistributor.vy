...
def ve_for_at(_user: address, _timestamp: uint256) -> uint256:
    """
    @notice Get the hTULIP balance for `_user` at `_timestamp`
    @param _user Address to query balance for
    @param _timestamp Epoch time
    @return uint256 hTULIP balance
    """
    ...
    return convert(max(pt.base + pt.bias - pt.slope * convert(_timestamp - pt.ts, int128), pt.base), uint256)
...
def _checkpoint_total_supply():
    ...
    for i in range(140):
        if t > rounded_timestamp:
            ...
            self.ve_supply[t] = convert(max(pt.base + pt.bias - pt.slope * dt, pt.base), uint256)
        t += DAY
...
def checkpoint_total_supply():
    """
    @notice Update the hTULIP total supply checkpoint
...
def _claim(addr: address, ve: address, _last_token_time: uint256) -> uint256[N_COINS]:
    ...
    for i in range(150):
        ...
        if day_cursor >= user_point.ts and user_epoch <= max_user_epoch:
            ...
            balance_of: uint256 = convert(max(old_user_point.base + old_user_point.bias - dt * old_user_point.slope, old_user_point.base), uint256)
...
def burn(_coin: address) -> bool:
    """
    @notice Receive TULIP into the contract and trigger a token checkpoint
    @param _coin Address of the coin being received (must be TULIP)
    @return bool success
    """
    assert not self.is_killed
...
def kill_me():
    ...
    @dev Killing transfers the entire TULIP balance to the emergency return address
